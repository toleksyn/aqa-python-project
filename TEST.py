...
class SearchResults:
    def verify_search_results_at_least(self, number_of_results):
        browser.all(by.xpath("//a[contains(@class, 'goods-tile__heading')]")).should(
    ...
    def add_product_to_cart(self, number_of_results):
        add_to_cart_buttons = browser.all(
...
class CartModal:
    def verify_cart_modal_is_open(self):
        ...
        place_order_button = browser.element(
            by.xpath('//a[@class="button button_size_large button_color_green cart-receipt__submit ng-star-inserted"]'))
